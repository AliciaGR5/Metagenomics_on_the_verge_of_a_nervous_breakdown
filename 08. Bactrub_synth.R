...
ma = matrix(0, nrow=length(coolfakebin2sample), ncol=3, dimnames= list(names(coolfakebin2sample), c('LyeJ (K20616)',
                                                                                                     'CrtD (K20611)',
...
for(sample in names(sample2coolfakebin)) {
  ...
  for(b in fakebins) {
    ...
    if ("K20616" %in% rownames(x$functions$KEGG$copy_number)){ # Sometimes the gene you are looking for is not in each bin so it turns out as an error
...
write.table(ma, file = "Bacterioruberin_synthesis_Job1_matrix.txt",sep = "\t")
...
write.table(ma, file = "Bacterioruberin_synthesis_Job1_df.txt",sep = "\t")
...
ggplot(ma, aes(x = Genes, y = Bins, fill = Copy_number)) + geom_tile(color="white") + # Plot heatmap
  ...
  # 
  # 
  xlab(label = "Bacterioruberin synthesis") +
  ylab(label = NULL) +
...
ggsave("Bacterioruberin_synthesis_Job1.png", width = 30, height = 25, units = "cm", bg = "transparent", dpi = 300)
ggsave("Bacterioruberin_synthesis_Job1.pdf", width = 30, height = 25, units = "cm", bg = "transparent", dpi = 300)
