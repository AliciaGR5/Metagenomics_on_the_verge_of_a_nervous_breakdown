...
###########################################
## BCAROTENE / BACTERIORUBERIN SYNTHESIS ##
###########################################
...
ma = matrix(0, nrow=length(coolfakebin2sample), ncol=7, dimnames= list(names(coolfakebin2sample), c('CrtB (K02291) // AL-2 (K17841)',
                                                                                                    ...
                                                                                                    'LyeJ (K20616)',
                                                                                                    'CrtD (K20611)',
                                                                                                    'CruF (K08977)'))) # Genes we want to work with
...
for(sample in names(sample2coolfakebin)) {
  ...
  for(b in fakebins) {
    ...
    if ("K20616" %in% rownames(x$functions$KEGG$copy_number)){ 
      ma[b,'LyeJ (K20616)'] = ma[b,'LyeJ (K20616)'] + x$functions$KEGG$copy_number['K20616',]} # Value of copy number will be added to our matrix
    
    if ("K20611" %in% rownames(x$functions$KEGG$copy_number)){
      ma[b,'CrtD (K20611)'] = ma[b,'CrtD (K20611)'] + x$functions$KEGG$copy_number['K20611',]}
    
    if ("K08977" %in% rownames(x$functions$KEGG$copy_number)){
      ma[b,'CruF (K08977)'] = ma[b,'CruF (K08977)'] + x$functions$KEGG$copy_number['K08977',]}
...
write.table(ma, file = "Bcarotene_Bacterioruberin_synthesis_Job1_matrix.txt",sep = "\t")
...
write.table(ma, file = "Bcarotene_Bacterioruberin_synthesis_Job1_df.txt",sep = "\t")
...
ggplot(ma, aes(x = Genes, y = Bins, fill = Copy_number)) + geom_tile(color="white") + # Plot heatmap
  ...
  # 
  # 
  xlab(label = "Carotenoids synthesis") +
  ylab(label = NULL) +
...
ggsave("NEWBcarotene_Bacterioruberin_synthesis_Job1.png", width = 30, height = 25, units = "cm", bg = "transparent", dpi = 300)
ggsave("NEWBcarotene_Bacterioruberin_synthesis_Job1.pdf", width = 30, height = 25, units = "cm", bg = "transparent", dpi = 300)
